%% ラベル値ごとに3次元ラドン変換で２次元投影を作成する（球面直交座標）
...
%   
%   output: ProjEachComponent{Label}(X,Y,theta,phi)
% 
% 

function ProjEachComponent = VerticesProjectionEachLabel3( Model, theta, phi )
%% 投影角度についての準備
NT = length(theta);
NP = length(phi);
harfNP = floor(NP/2);
...
for it = 1:NT
    for ip = 1:harfNP
        ...
        for il = 1:NL
            ...
            ProjEachComponent{il}(:,:,it,ip) = squeeze(sum(Component,1));% 投影計算
%             if mod(elevation_ia + pi/4, pi/2) == 0    % 45度角かどうか
%                 ProjEachComponent{il}(:,it,ip) = ProjEachComponent{il}(:,it,ip) / 1.0962; % 回転移動による誤差の打消し（マジックナンバー）
%             end
        end
    end
end
