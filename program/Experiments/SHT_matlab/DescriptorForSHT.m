...
%       nDivide / 特徴量を何回分割するか。低周波成分カット
%       orderMax / 特徴量計算での球面調和変換の次数上限
%
% 出力 : DescriptorsCell{Label}(高周波フィルター後のサイズ NZ2, SHTによる各次数の係数)
% 
%%
function [DescriptorsCell] = DescriptorForSHT(Projection, nDivide, orderMax)

[NZ,NT,NP] = size(Projection{1});

% 球面調和関数の計算
% 実際はmainかグローバルで用意した方が良い
...
if iscell(Projection) 
    ...
    for ic = 1:NC
        ...
        DescriptorsCell{ic} = ComputeDescriptor(Proj, SHs, nDivide); % 特徴量計算にぶんなげ
    end
else
    %%% 部品ごとにラベル分けされていないとき
    DescriptorsCell = ComputeDescriptor(Projection, SHs, nDivide); % 特徴量計算にぶんなげ
end
end
...
% 符号と正規化
sign = (-1)^((m+abs(m))/2);
term = sqrt( (2*l+1) / 4*pi * factorial(l-abs(m)) / factorial(l+abs(m)) );

% 複素数 球面調和関数
Ylm = zeros(polarNum, azimuthNum);
Ylm(:,:) = sign * term * Plm .* exp(1i * m * phi);
