%% モデル間の特徴量ユークリッド距離比較（SHT）
% EuclideanDistanceEmphasizedModel.m の改造
...
%                   DescriptorsCell{Label}(高周波フィルター後のサイズ NZ2, SHTによる各次数の係数)
% 
...
% ∋ DQ, DD （ある部品の特徴量行列（回転平行移動について不変））
%
%%
function [EachMinimumsTable, EachComponentNumber, EachDistanceTable, EachSumDistanceTable] = ...
    EuclideanDistanceEmphasizedModel2(DescriptorQ, DescriptorD)
%% 定数,セル配列の用意
           ...
           DQ = DesQ{iqc};                              % クエリの部品（の特徴量行列）を１つ取り出す
            ...
            for idc = 1:NDC     % そのDBモデルの部品についてループ
                DD = DesD{idc};                                 % DBの部品（の特徴量行列）を１つ取り出す
                                                                % 各行に、各インデックスごとの係数列が入った行列
                DistanceDescriptor = sum(sum((DQ - DD).^2));            % ユークリッド距離 
                DistanceTable(iqc,idc) = DistanceDescriptor;            % DQ,DD間の距離（非類似度）に使う
