...
% 投影枚数 13*27
% 最大展開次数 5
% 投影画像計算時間：：23.1002[sec]
% 特徴量計算時間：：1797.6464[sec]
% ユークリッド距離計算時間：：0.03073[sec]
% 実験時間：1822.0272[sec]
% 正答数：1/1

%% パラメーター設定
orderMax = 5;       % 球面調和展開の最大展開次数（初期値: 5）
elevationNum = 13;  % 緯度刻み数(始点と終点を含む)（初期値: 13）
azimuthNum = 27;    % 経度刻み数(始点と終点を含む)（初期値: 27）

theta = 0 : pi/(elevationNum - 1) : pi;
phi = 0 : 2*pi/(azimuthNum - 1) : 2*pi;

% tic; SHT(projection3d(Clutch64001,theta,phi),5); toc
...
NumberOfHalving = 1;    % 特徴量を分割する回数。ピークを中心に特徴量を半分のサイズになるように切り取る（高周波削減）
...
disp('各データベースモデルの回転パターンを適用中...');
DatabaseRotAzi = [0];
DatabaseRotEle = [30-45];
DatabaseTranslation = [0 0 0];
%}
...
[EachMinimumsTable, EachComponentNumber, EachDistanceTable, EachSumDistanceTable] = EuclideanDistanceEmphasizedModel2(DescriptorQ, DescriptorD);
