...
% 
% Output: ProjectionEmphasizedComponent{Label}(Z,theta,phi) / そのラベル値の部品が強調された投影集合
% 
% ProjectionEachComponent = {
%   Projection(Z,theta,phi),
%   Projection(Z,theta,phi),
%   Projection(Z,theta,phi), ... }
% 
% 
%%
function [ProjectionEmphasizedComponent] = ProjectionEmphasizedModel2(Model, theta, phi, EmphasisValue)
%% ラベル値ごとに投影
...
for iE = 1:NL
    ...
    ReOP = zeros(size(OthersProjection));
    for iP = 1:NumberOfVertices
        ReEP(:,:,iP) = EmphasizedProjection(:,:,iP) / VolumeEP(iP); % 元々のラベル値で撮った投影を体積値で割る（逆数を掛ける）
